...
Spritesheet.prototype.composite = function (forcedWidth, forcedHeight) {
  forcedWidth = forcedWidth || this.width;
  forcedHeight = forcedHeight || this.height;

  this.buffer = new ImageBuffer({
    width: Math.max(this.width, forcedWidth),
    height: Math.max(this.height, forcedHeight)
  });
